...
import {addFoodUrl, batchDeleteFoodUrl, deleteFoodUrl, getFoodListUrl, updateFoodUrl} from "@/api/food.js";
...
export function useFood() {
    ...
    const getTableData = () => {
        loading.value = true
        delayRequest(() => getFoodList(), 5, 500)
    }
    const add = (value) => {
        addFoodUrl(value)
    ...
    const handleBatchDelete = () =>
        confirmBox('确定删除选中的数据吗？', '提示', 'warning', () =>
            batchDeleteFoodUrl(selection.value.map((item) => item['id']))
                .then(({data}) => result(data))
                .catch(({response}) => ElNotification.error(response.data.message))
        )
