...
import {batchDeleteShareUrl, deleteShareUrl, getShareListUrl, updateShareUrl} from "@/api/share.js";
...
export function useShare() {
    ...
    const columns = ref([
        ...
        {label: '昵称', status: true},
        {label: '图片', status: true},
        {label: '内容', status: true},
        {label: '发布时间', status: true},
    ...
    const getTableData = () => {
        loading.value = true
        delayRequest(() => getShareList(), 5, 500)
    }
    const update = (value) => {
    ...
    const handleBatchDelete = () =>
        confirmBox('确定删除选中的数据吗？', '提示', 'warning', () =>
            batchDeleteShareUrl(selection.value.map((item) => item['id']))
                .then(({data}) => result(data))
                .catch(({response}) => ElNotification.error(response.data.message))
        )
